from .arch import get_norm_layer
...
def get_norm_param(model):
    import operator
    import functools

    norm = get_norm_layer(model)
    para = 0
    for op in norm:
        if isinstance(op, nn.LayerNorm):
            para += sum([functools.reduce(operator.mul, i.size(), 1) for i in op.parameters()])
