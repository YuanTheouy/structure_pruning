...
#       4. 指定目标稀疏度: ./scripts/searchPPOL7.sh --target-sparsity 0.7
#       5. 指定训练轮数: ./scripts/searchPPOL7.sh --train-episodes 8000
#       6. 兼容旧参数: ./scripts/searchPPOL7.sh --preserve-ratio 0.3  (会自动转换为稀疏度0.7)
# =================================================================================
...
EXP_ID=0           # 默认实验配置ID
TARGET_SPARSITY=0.7 # 默认目标稀疏度 (70%参数被剪枝)
TRAIN_EPISODES=8000 # 默认训练轮数
ENABLE_DOWNSTREAM=false # 默认开启下游任务评估
...
GRADUAL_PRUNING_END_EPISODE=2000 # 渐进式剪枝结束的episode
GRADUAL_INITIAL_SPARSITY=0.0 # 渐进式剪枝的初始稀疏度
SHOW_HELP=false
...
if [ "$SHOW_HELP" = true ]; then
    ...
    echo "  --target-sparsity S 指定目标稀疏度 (默认: 0.7，即剪枝70%参数)"
    ...
    echo "  初始稀疏度由 --gradual-initial-sparsity 参数决定 (默认: 0.0)。"
    ...
    echo "  $0 --target-sparsity 0.7 --gradual-initial-sparsity 0.1 --gradual-pruning-end-episode 3000 # 完整的渐进式剪枝配置"
    exit 0
fi
...
# MODEL_PATH="/public/home/weijiateng2023/model/llama-2-7b-hf"
...
EXPORT_PATH="./checkpoints/llama27b_ppo_${SUFFIX}.pth.tar"
...
# 对于Python脚本，我们传递第一个GPU ID作为主GPU，但CUDA_VISIBLE_DEVICES包含所有GPU
MAIN_GPU_ID=$(echo ${GPU_ID} | cut -d',' -f1)
echo "=> [GPU信息] 主GPU ID: ${MAIN_GPU_ID}" 

# --- 7. 显示实验配置信息 ---
echo "=================================================================="
echo "   启动 AMC-LLM 剪枝搜索 - llama-2-7b-hf 实验 #${EXP_ID}"
echo "=================================================================="
echo "    实验配置:"
