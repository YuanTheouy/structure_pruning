...
TOP_K=${TOP_K:-"50"}
REPLAY_TOP_K=${REPLAY_TOP_K:-"5"}
PROMOTION_MIN_CANDIDATES=${PROMOTION_MIN_CANDIDATES:-"${REPLAY_TOP_K}"}
MIN_PREFIX_STEP=${MIN_PREFIX_STEP:-"300"}
CARRY_FORWARD_MODE=${CARRY_FORWARD_MODE:-"pas"}
...
GRADUAL_STAGE_SPARSITIES=${GRADUAL_STAGE_SPARSITIES:-"${STAGES}"}
USE_DATASET_GROWTH=${USE_DATASET_GROWTH:-"true"}
...
RUN_ID_DEFAULT="progressive_pas_lookahead_seed${SEED}_ep${TRAIN_EPISODES}_gpu${SEARCH_GPU}_fixed${DATASET_INITIAL_RATIO}_${GRADUAL_PRUNING_SCHEDULE}"
...
OUT_ROOT=${OUT_ROOT_OVERRIDE:-"/workspace/ckpts/pas_progressive_lookahead/${MODEL_NAME}_seed${SEED}_ep${TRAIN_EPISODES}_fixed${DATASET_INITIAL_RATIO}_${GRADUAL_PRUNING_SCHEDULE}"}
...
MIN_PREFIX_STEP=${MIN_PREFIX_STEP}
CARRY_FORWARD_MODE=${CARRY_FORWARD_MODE}
USE_DATASET_GROWTH=${USE_DATASET_GROWTH}
DATASET_INITIAL_RATIO=${DATASET_INITIAL_RATIO}
DATASET_FINAL_RATIO=${DATASET_FINAL_RATIO}
GRADUAL_PRUNING_SCHEDULE=${GRADUAL_PRUNING_SCHEDULE}
GRADUAL_STAGE_SPARSITIES=${GRADUAL_STAGE_SPARSITIES}
RUN_ROOT=${RUN_ROOT}
...
"${PYTHON_BIN}" scripts/pas_progressive_lookahead_replay.py \
  ...
  --promotion-min-candidates "${PROMOTION_MIN_CANDIDATES}" \
