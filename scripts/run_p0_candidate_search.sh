...
GRADUAL_PRUNING_SCHEDULE=${GRADUAL_PRUNING_SCHEDULE:-"cubic"}
...
if [[ "${USE_GRADUAL_PRUNING}" == "true" ]]; then
  ...
  if [[ -n "${GRADUAL_STAGE_SPARSITIES}" ]]; then
    GRADUAL_FLAGS+=(--gradual_stage_sparsities="${GRADUAL_STAGE_SPARSITIES}")
  fi
fi
