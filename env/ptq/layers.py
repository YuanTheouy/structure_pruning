...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self,
        ...
        # self.qact = QAct(scale=self.scale_a, zero=self.zero_a, bit=self.bit)
        ...
        self.quantizer = build_quantizer(self.quantizer_str, self.scale_a,
                                         self.zero_a, self.bit, self.module_type)
        self.quantizer_w = build_quantizer(self.quantizer_str, self.scale_w,
                                         self.zero_w, self.bit, self.module_type)
        self.quantizer_b = build_quantizer(self.quantizer_str, self.scale_b,
                                         self.zero_b, self.bit, self.module_type)
    ...
    def forward(self, x):
        ...
        #     )
        # x = self.quantizer(x)
        weight = self.quantizer_w(self.weight).cuda()
        # if self.bias is not None:
        #     bias = self.quantizer_b(self.bias).cuda()
        # else:
        #     bias = self.bias
        # print(bias)
        bias = self.bias
...
class QAct(nn.Module):
    ...
    def __init__(self,
        ...
        # self.observer = build_observer(self.observer_str, self.module_type,
        #                                self.bit, self.calibration_mode)
        self.quantizer = build_quantizer(self.quantizer_str, self.scale,
                                         self.zero, self.bit, self.module_type)
